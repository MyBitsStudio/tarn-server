...
public class GroupIronmanBank extends ItemContainer {
	...
	public GroupIronmanBank open(Player player) {
		...
		player.setIronBanking(true).setInputHandling(null);
	...
	public GroupIronmanBank refreshInterface(Player player) {
		...
		player.getPacketSender().sendString(106015, "" + this.getValidItems().size());
		player.getPacketSender().sendString(106016, "" + this.capacity());
		player.getPacketSender().sendItemContainer(INTERFACE_ID_REAL, this);
	...
	public static void depositItems(Player p, ItemContainer from, boolean ignoreReqs) {
		...
		for (Item it : from.getValidItems()) {
			...
			p.getGroupIronmanBank().add(it, false);
