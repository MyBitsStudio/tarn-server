...
import com.ruse.world.content.upgrade.UpgradeInterfaceData;
...
public class PacketSender {
    ...
    public PacketSender updateProgressSpriteBar(int interfaceId, int progress, int maxPercentage) {
        PacketBuilder out = new PacketBuilder(204);
        out.putInt(interfaceId);
        out.putInt(progress);
        out.putInt(maxPercentage);
        player.getSession().queueMessage(out);
        return this;
    }
...
//    }
    ...
    public PacketSender sendRaidsTabIcon(boolean show) {
        PacketBuilder out = new PacketBuilder(103);
        out.put(show ? 1 : 0);
        player.getSession().queueMessage(out);
        return this;
    ...
    public PacketSender sendTabs() {
        ...
        sendTabInterface(GameSettings.MAGIC_TAB, 123000);
    ...
    public PacketSender sendItemContainer(Item[] container, int interfaceId) {
        PacketBuilder out = new PacketBuilder(53, PacketType.SHORT);
        out.putInt(interfaceId);
    ...
    public PacketSender sendItemContainer(List<Item> container, int interfaceId) {
        PacketBuilder out = new PacketBuilder(53, PacketType.SHORT);
        out.putInt(interfaceId);
    ...
    public PacketSender sendItemContainer(UpgradeInterfaceData[] container, int interfaceId) {
        PacketBuilder out = new PacketBuilder(53, PacketType.SHORT);
        out.putInt(interfaceId);
        out.putShort(container.length);
        for (UpgradeInterfaceData item : container) {
            Item item1 = item.getToUpgrade();
            if (item1 == null) {
                out.put(0);
                out.putShort(0, ValueType.A, ByteOrder.LITTLE);
                continue;
            ...
            if (item1.getAmount() > 254) {
                out.put((byte) 255);
                out.putInt(item1.getAmount(), ByteOrder.INVERSE_MIDDLE);
            } else {
    ...
    public PacketSender createGroundItem(GroundItem gi) {
    ...
    public PacketSender removeGroundItem(GroundItem gi) {
        sendPosition(new Position(gi.getPosition().getX(), gi.getPosition().getY()));
