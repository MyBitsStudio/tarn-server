...
public class NPC extends Character {
    ...
    public int getAggressionDistance() {
        ...
        if (Nex.nexMob(id)) {
            return 60;//can you open the search
        } else if (id == 2896) {
            return 50;
        } else if(id == 9014 || id == 9017 || id == 3305 || id == 9904 || id == 9904 || id == 9906 || id == 9907 || id == 9908) {//globalbosses
            return 50;
        }
    ...
    public void removeInstancedNpcs(Location loc, int height, Player player) {
        ...
        for (int i = 0; i <= checks; i += 2) {
            if (getPosition().getX() >= loc.getX()[i] && getPosition().getX() <= loc.getX()[i + 1]) {
                if (getPosition().getY() >= loc.getY()[i] && getPosition().getY() <= loc.getY()[i + 1]) {
                    if (getPosition().getZ() == height && (player == null || (getSpawnedFor() != null && Arrays.stream(getSpawnedFor()).anyMatch(p -> p.getIndex() == player.getIndex())))) {
                        World.deregister(this);
                    }
                }
            }
