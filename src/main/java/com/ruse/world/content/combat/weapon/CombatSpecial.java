...
public enum CombatSpecial {
	...
	MINIGUN(new int[] { 4017, 19135, 19136, 19137, 20173, 671, 20553, 19843,18636,8088,8411,22148,5012,12843,5010,5011,22113,21606,8001,22089,9929,14056,15785,8253, 22083 }, 33, 1.01, 2.01, CombatType.RANGED,
...
//			player.getSkillManager().setCurrentLevel(Skill.MAGIC, 200);
		...
		@Override
		public CombatContainer container(Player player, Character target) {
			throw new UnsupportedOperationException("Soldier Lance does not have a special attack!");
		}
	},
...
//								.sendMessage("Your hammer forces some of your opponent's defences to break.");
			...
			return new CombatContainer(player, target, 1, CombatType.MELEE, true) {
				...
				public void onHit(long damage, boolean accurate) {
					if (target.isPlayer()) {
						...
						NPC t = (NPC) target;
						long currentHealth = t.getConstitution() / 100;
	...
	HAND_CANNON(new int[] { 4017, 15241, 19843,18636,8088,8411,22148,5012,12843,5011,22113,5010,22089,8001,21606,9929,14056,8253, 13956, 19135 }, 45, 1.45, 1.15, CombatType.RANGED, WeaponInterface.SHORTBOW) {
...
//					player.getSkillManager().getCurrentLevel(Skill.ATTACK) - 7);
	...
	DRAGON_SPEAR(new int[] { 1249, 1263, 5716, 11716 }, 25, 1, 1, CombatType.MELEE, WeaponInterface.SPEAR) {
		@Override
		public CombatContainer container(Player player, Character target) {
			player.performAnimation(new Animation(1064));
