...
public class InfernalDemon implements CombatStrategy {
    ...
    private static final Animation MAGIC = new Animation(14221);
    private static final Animation MELEE = new Animation(14221);
    private static final Animation SCREAM = new Animation(14221);
    private static final Animation SUMMON = new Animation(14221);
    ...
    public boolean customContainerAttack(Character entity, Character victim) {
        ...
        if (Locations.goodDistance(demon.copy(), victim.copy(), 100)) {
            if (rng >= 90) {
    ...
    private void magicAttack(NPC npc, Character victim) {
        ...
        TaskManager.submit(new Task(1, npc, false) {
            ...
            public void execute() {
                    if (tick == 0) {
                            new Projectile(npc, victim, 1339, 50, 35, 20, 28, 0).sendProjectile();
                            victim.dealDamage(new Hit(100, Hitmask.RED, CombatIcon.MAGIC));
                    } else if (tick == 1) {
        ...
        TaskManager.submit(new Task(1, npc, false) {
            ...
            public void execute() {
                if (tick == 0) {
                    ...
                    if(Locations.goodDistance(target, npc, 25)) {
                        //npc.forceChat("Your gods wont help you here!!");
                        //target.sendMessage("Your prayers got knocked down!");
