...
public class CombatContainer {
	...
	private ContainerHit[] prepareHits(int hitAmount) {
		...
		if (attacker.isPlayer() && ((Player) attacker).isSpecialActivated()) {
			if (((Player) attacker).getCombatSpecial() == CombatSpecial.DRAGON_CLAWS && hitAmount == 4) {
				long first = array[0].getHit().getDamage();
				if (first > 360) {
					first = 360 + Misc.getRandom(10);
				}
				long second = first <= 0 ? array[1].getHit().getDamage() : (int) (first / 2);
				long third = first <= 0 && second > 0 ? (int) (second / 2)
						: first <= 0 && second <= 0 ? array[2].getHit().getDamage() : Misc.getRandom((int) second);
				long fourth = first <= 0 && second <= 0 && third <= 0
	...
	public void setModifiedDamage(long modifiedDamage) {
		this.modifiedDamage = modifiedDamage;
	}

	public long getModifiedDamage() {
