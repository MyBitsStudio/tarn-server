...
public class UpgradeInterface {
    ...
    public void upgrade() {
      ...
      //  System.out.println("Found item "+item.getSlot());
        Item newItem;
        if(item.getEffect() == ItemEffect.NOTHING) {
            newItem = getData().getUpgradedItem();
        }
        else {
            newItem = new Item(getData().getUpgradedItem().getId(), getData().getUpgradedItem().getAmount(), item.getEffect(), item.getBonus());
        }
    ...
    public void sendRequirements() {
        ...
        for(int i = 0; i < getData().getRequiredItems().length; i++) {
            player.getPacketSender().sendItemOnInterface(19422, getData().getRequiredItems()[i].getId(), i, getData().getRequiredItems()[i].getAmount(), getData().getRequiredItems()[i].getBonus());
        }
    }

    public void setCategory(UpgradeInterfaceData[] category) {
    ...
    public void setData(UpgradeInterfaceData data) {
        ...
        player.getPacketSender().sendItemOnInterface(19428, getData().getUpgradedItem().getId(), 0, getData().getUpgradedItem().getAmount(), getData().getUpgradedItem().getBonus());
    }
